-- Allow exact local times without a fully resolved timezone chain.
-- Hotel check-in/out often has HH:mm but no IANA zone; extraction keeps that as
-- unresolved. Confirmation must accept the same shape so candidates can become
-- travel items without inventing a zone.
...
begin
  ...
  if v_precision = 'exact_time' then
    ...
    if v_resolution = 'resolved' then
      ...
      -- unresolved / ambiguous / nonexistent: keep printed local clock time without inventing a zone.
      ...
      -- If a full chain is present even under unresolved, still check internal consistency.
...
begin
  ...
  -- Same calendar day with local clocks only (unresolved exact times).
