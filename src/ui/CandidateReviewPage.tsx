...
export function CandidateReviewPage() {
  ...
  function updateStartDate(value: string) {
    const payload = parseDraft() ?? {};
    const start =
      payload.start_time && typeof payload.start_time === "object" && !Array.isArray(payload.start_time)
        ? (payload.start_time as Record<string, unknown>)
        : {};
    payload.start_time = {
      ...start,
      local_date: value,
      precision: start.precision ?? "date_only",
      resolution_status: start.resolution_status ?? "date_only"
    };
    setDraft(JSON.stringify(payload, null, 2));
    setDirty(true);
    setMessage(null);
  ...
  async function save(): Promise<CandidateMutationResult | null> {
    ...
    setMessage(
      result.kind === "conflict" ? result.message : "message" in result ? result.message : "Korrektur konnte nicht gespeichert werden."
    );
    return result;
  }
  ...
  async function confirm() {
    ...
    setMessage(
      result.kind === "conflict"
        ...
        : "message" in result
          ? result.message
          : "Der Speicherstatus konnte nicht bestätigt werden."
    );
  }
  ...
  async function discard() {
    if (
      !record ||
      !window.confirm(
        "Entwurf verwerfen? Original und Korrekturen bleiben für die Nachvollziehbarkeit erhalten; es wird kein Ereignis erzeugt."
      )
    ) {
      return;
    }
    ...
    if (result.kind === "discarded") navigate("/documents", { replace: true });
      ...
      setMessage(
        result.kind === "conflict" ? result.message : "message" in result ? result.message : "Entwurf konnte nicht verworfen werden."
      );
    }
  }
  ...
  if (!record || record.candidate.status !== "draft") {
    return (
      <section className="state-card" role="alert">
        ...
        <button type="button" className="primary-button state-action" onClick={() => navigate("/documents")}>
          Zu den Dokumenten
        </button>
      </section>
    );
    ...
    <section className="form-card candidate-review" aria-labelledby="candidate-review-title">
      <div className="page-heading">
        <div>
          ...
          <p className="muted stack-gap-sm">
            Quelle: {record.document.originalFileName}
          </p>
        </div>
        <button type="button" className="link-button" onClick={() => navigate("/documents")}>
          Zurück
        </button>
      </div>
      {message ? (
      ...
      <div className="candidate-review-layout">
        <div>
          <div className="field-grid">
            <div className="field">
              ...
              <select
                ...
                onChange={(event) => updateCore("event_type_code", event.target.value)}
              >
                {(Object.keys(eventTypeLabels) as EventTypeCode[]).map((code) => (
            ...
            <div className="field">
              <label htmlFor="candidate-start">Startdatum *</label>
              <input
                id="candidate-start"
                type="date"
                value={typeof start.local_date === "string" ? start.local_date : ""}
                onChange={(event) => updateStartDate(event.target.value)}
              />
            </div>
          </div>
          <div className="field">
            <label htmlFor="candidate-title">Titel *</label>
            <input
              id="candidate-title"
          ...
          <div className="field">
            <label htmlFor="candidate-payload">Vollständiger geprüfter Ereignisstand</label>
            <textarea
              ...
              aria-describedby="candidate-payload-hint"
            />
            <p id="candidate-payload-hint" className="field-hint">
              Alle gemeinsamen, typspezifischen und wiederholbaren Felder einschließlich <code>segments</code> können hier
              ergänzt, entfernt und neu geordnet werden.
            </p>
          </div>
          <div className="form-actions">
            <button type="button" className="secondary-button" onClick={() => void save()} disabled={isSaving}>
              {isSaving ? "Wird gespeichert …" : "Korrekturen speichern"}
            </button>
            <button type="button" className="primary-button" onClick={() => void confirm()} disabled={isSaving}>
              Ereignis bestätigen
            </button>
            <button type="button" className="link-button destructive-link" onClick={() => void discard()} disabled={isSaving}>
              Entwurf verwerfen
            </button>
          </div>
        </div>
        <aside className="candidate-evidence" aria-labelledby="candidate-evidence-title">
          <div className="section-heading">
            <div>
              <p className="eyebrow">Herkunft und Unsicherheit</p>
              <h2 id="candidate-evidence-title">Erkannte Felder</h2>
            </div>
            <button type="button" className="secondary-button" onClick={() => void openOriginal()}>
              Original öffnen
            </button>
          </div>
          {record.candidate.warnings.length ? (
            <ul className="candidate-warning-list">
              {record.candidate.warnings.map((warning, index) => (
                <li key={`${warning.code}-${index}`}>
                  <strong>{warning.severity === "blocking" ? "Blockierend: " : "Prüfen: "}</strong>
                  {warning.message}
                </li>
              ))}
            </ul>
          ...
          <ul className="candidate-field-list">
            {record.candidate.fields.map((field) => (
              <li key={`${field.fieldPath}:${field.occurrenceKey}`}>
                <strong>
                  {formatFieldPath(field.fieldPath)}
                  {field.occurrenceKey ? ` · ${field.occurrenceKey}` : ""}
                </strong>
                ...
                <span className="muted">
                  {field.provenance === "explicit"
                    ...
                    : field.provenance === "inferred"
                      ? "Abgeleitet – bitte prüfen"
                      : "Unbekannt"}
                  {field.confidence === null ? "" : ` · ${Math.round(field.confidence * 100)} %`}
                </span>
                {field.sourceLocator.map((locator, index) => (
                  <small key={`${locator.sourceHint}-${index}`}>
                    {locator.pageNumber ? `Seite ${locator.pageNumber}: ` : ""}
                    {locator.sourceHint}
                  </small>
                ))}
              </li>
          ...
          {originalUrl ? (
            <a className="secondary-button download-link" href={originalUrl} download={record.document.originalFileName}>
